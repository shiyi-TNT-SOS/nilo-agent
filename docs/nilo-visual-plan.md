...
3. 用户不用写专业资料，只回答几个问题：我想推广什么、适合谁、主要好处、有什么证明、希望观众做什么、哪些话不能说。系统把回答整理成项目资料。
4. 系统把对标套路匹配到用户项目，生成可拍口播脚本。
5. 用户选择脚本版本，准备标题、封面文案、标签、评论引导和素材清单。
6. 用户选择平台数字人，或上传本人授权采集视频创建数字人，并选择背景模板/上传项目素材。
7. 系统基于脚本、数字人、背景模板和项目素材合成口播视频，并做合规审核。
8. 审核通过后进入最终成片生成与记录。

### 发布设置选择器

首屏必须保留轻量发布设置，不要藏进高级区：

- 平台：抖音、小红书、视频号、B站。
- 比例：9:16 竖屏、1:1 方屏、16:9 横屏。
- 时长：30秒、60秒、90秒。

这些设置会写入任务状态，并进入成片素材包，后续用于脚本长度、数字人合成比例、字幕和 FFmpeg 成片参数。

### 填入项目的前台表达

这一步不要让小白理解“项目事实档案”。前台应表达为：**告诉 Nilo 你要推广什么**。

推荐字段：

- 我想推广：产品、服务、课程、工具、门店、社群或咨询。
- 适合的人：目标用户是谁，他们现在卡在哪里。
- 主要好处：能解决什么问题，交付什么结果。
- 真实证明：案例、截图、客户反馈、交付记录；没有可以先写暂无。
- 希望观众做什么：评论、私信、预约、领取资料、加微信等。
- 哪些话不能说：收益承诺、保证爆款、夸大效果、未授权素材等。

按钮表达：`AI 帮我整理`、`我自己补充`、`不会写，填个示例`、`保存我的推广信息`。

推广类型选项使用选择器表达：`AI 工具`、`课程`、`服务`、`门店`、`社群`、`咨询`。当前先用本地规则整理草稿，后续接 `/api/chat` 让模型根据一句话自动补全。

### 数字人背景规则

数字人视频的背景不来自对标视频，也不复刻对标视频画面。背景来源按优先级分为：

1. 默认背景模板：简洁演播室、产品展示台、办公场景、课程讲解板、透明/绿幕。
2. 用户上传的项目素材：产品图、网页截图、案例截图、课程海报、服务流程图，可作为背景展示或 B-roll。
3. 脚本内容匹配：讲痛点用干净背景，讲产品用产品/网页素材，讲案例用截图/数据，讲行动引导用 CTA 背景。
4. 后期合成：透明/绿幕模式后续接 FFmpeg，把数字人、字幕、背景、项目素材合成最终视频。

产品边界：对标视频只用于分析结构和节奏，不能作为数字人背景素材来源；任何人物、声音、图片、品牌素材都需要用户有使用授权。
